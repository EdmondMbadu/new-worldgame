...
- Candidate URLs come from Gemini Google Search grounding metadata, never generated prose. Each shared research pack considers at most 12 retrieved sources per category and one replacement pass of at most 4.
...
- News needs a supported date within 90 days. Funding eligibility needs page evidence; unknown cycles are labeled. Scores below 75, expired deadlines, known dead links and unverified pages are excluded. Select up to four per category and at most one per host; send fewer if necessary.
- Cache keys include pipeline version, context hash, and edition date. Shared weekly research is reused across recipients in the same edition; the latest verified pack is retained as a budget/provider fallback. Selection still rejects news older than 90 days and expired funding deadlines at rendering. Empty caches retry after 15 minutes. Old HTML-only caches are never reused.
- Cold research has a 400-second budget. Bulk continuation batches contain three recipients with at most three shared packs preparing concurrently. Single calls and the admin preview allow 540 seconds; delivery never retries a failed preparation synchronously within the same batch.
- Automated delivery sends every eligible, subscribed solution participant. A saved user selection wins; otherwise the participant's most recently updated solution is used. Administrators can still explicitly choose a different fallback rule or the opt-in-only mode.
- A weekly edition contains at most 18 topic or topic/region research packs. The live recipient corpus currently resolves to 18 packs for 287 eligible recipients. Per-recipient matching then selects up to four articles and four funding opportunities using the solution title, description, SDGs, location, prior delivery history, and a stable weekly rotation seed.
- Recipient history suppresses links delivered during the previous eight weeks when enough unseen verified sources exist. Funding must have official-page eligibility evidence; location-specific funding must explicitly serve the recipient's region or be genuinely global. One publisher can appear at most once in each section.
- The edition reserves at most $3.96 against a $4.25 Gemini budget. Actual input/output tokens, estimated cost, cache hits, cold packs, failures, and budget fallbacks are recorded with the send log. Reaching the budget uses the latest eligible verified pack and never removes a recipient.
- Gemini responses have explicit output limits. The dashboard, feedback, video, additional-resource, unsubscribe, and source URLs are unchanged.
