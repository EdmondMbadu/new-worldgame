...
### Plays everywhere (revision 7)

Revision 7 makes the game load quickly on slow and metered connections and run smoothly on older phones. On capable devices, High looks the same as before.

- **Smaller first drive.** From opening the page to the first drive, the game now downloads about 1.7 MB (menu art, code, physics, surfaces and characters), down from about 11 MB. Clinic compounds are built from code rather than downloaded. The staff model is meshopt-compressed. Textures and key art are WebP. The physics engine streams and compiles its WebAssembly while it downloads. A few seconds on the menu prefetch the drive's code and surfaces, unless data saver is on.
- **Connection-aware extras.** Music streams 96 kbps copies unless the connection is fast and unmetered. Sharper road and ground textures arrive a few seconds into the drive, and only on fast connections; the base set is always complete.
- **Offline repeat visits.** A small service worker caches the game's files after the first visit. Its cache name is a hash of the build, so every deploy is picked up; audio range requests go straight to the network.
- **Graphics tiers.** Settings offer Auto, High, Balanced and Light. Auto starts from the GPU, memory, cores and input type. High keeps MSAA, sun shafts, bloom and terrain and headlight shadows. Balanced uses FXAA and drops the two costliest shadows while keeping the golden-hour shafts and bloom. Light targets older phones: it drops post effects and renders less vegetation. On Auto, a frame governor first lowers resolution (down to 60 %) when frames run long, then steps down one tier; it never steps up mid-drive, so the look does not flicker.
- **No hitches.** Shaders are compiled before the drive starts, against the same render target they draw to ("Lighting the road"). Vehicle beams share one pooled spotlight, so the light count never changes. Each person is one draw call. Road geometry lookups are cached per station.
...
- `src/world.ts`, `art.ts`, `foliage.ts`, `surfaces.ts`, `living-world.ts`: Three.js renderer, chunked terrain, shaped truck meshes, skinned characters, wind animation, roadside villages, sky-baked image lighting, rain, surface spray, headlights, clinic interiors, unloading and camera choreography.
...
- `src/night.ts`, `road-art.ts`, `clinic-assets.ts`, `clinic-architecture.ts`, `staff.ts`: night profiles, hazard markers/water, chapter-specific clinic compounds and animated staff.
