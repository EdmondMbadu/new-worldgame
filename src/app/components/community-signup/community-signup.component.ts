...
export class CommunitySignupComponent implements OnInit {
  ...
  // extra profile info (useful for ops)
  schoolCountry = '';
  ...
  ngOnInit(): void {
    // If logged in, check if they already own a paid workspace
    this.afAuth.authState
      .pipe(
        switchMap((u) => {
          ...
          return this.auth.userDoc$(u.uid).pipe(
            switchMap((userDoc) => {
              if (!userDoc?.schoolId) return of({ userDoc, workspace: null });
              return combineLatest([
                of(userDoc),
                this.auth.schoolDoc$(userDoc.schoolId),
              ]).pipe(map(([userDoc, workspace]) => ({ userDoc, workspace })));
            })
          );
        })
      )
      .subscribe((res) => {
        if (!res) return;
        const { userDoc, workspace } = res;
        if (
          workspace?.billing?.paymentStatus === 'paid' &&
          workspace?.ownerUid === userDoc?.uid
        ) {
          this.blocked = true;
          this.blockedReason = `You already manage "${workspace.name}".`;
          // Optionally redirect:
      ...
      if (user) {
        ...
        if (user.schoolId) {
          ...
          if (schoolSnap?.ownerUid === user.uid) {
            this.blocked = true;
            this.blockedReason = `You already own a community workspace "${schoolSnap.name}". You can only create one workspace.`;
          }
        }
      } else {
        this.isExistingUser = false;
  ...
  private friendly(msg: string) {
    ...
    if (msg.includes('already own a school') || msg.includes('already own a community workspace'))
      return 'You already own a community workspace. You can only create one workspace.';
  ...
  async createCommunityAccount() {
    ...
    try {
      if (this.isExistingUser && this.existingUser?.uid) {
        // Upgrade existing user to community admin workspace
        ...
        // Create new community workspace account
